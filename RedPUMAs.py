...
def dp_errs2(hist,fnames,eps): #return l1errs, fairerrs 
  #hist=prep_hist(df)
  #hist_r=count_scale_df(hist  l1errs=[]
  fairerrs=[]
  for iter in range(100):
    print(iter)
    hist_dp=dp_counts_df(hist,eps)
...
pumas=list(df['PUMA'].unique())
'''
l1errs1tot=np.zeros(100)
l1errs2tot=np.zeros(100)
l1errs3tot=np.zeros(100)
l1errs5tot=np.zeros(100)
fairerrs1tot=np.zeros(100)
fairerrs2tot=np.zeros(100)
fairerrs3tot=np.zeros(100)
fairerrs5tot=np.zeros(100)
###
l1errsx2tot=np.zeros(100)
l1errsx3tot=np.zeros(100)
l1errsx6tot=np.zeros(100)
fairerrsx2tot=np.zeros(100)
fairerrsx3tot=np.zeros(100)
fairerrsx6tot=np.zeros(100)
'''
for puma in pumas:
  #for eps in [1,2,3,5]:
  hist1=prep_hist(df[df['PUMA']==puma])
  #puma=pumas[pumaind]
  fnameses=[['AGE','OWNERSHP'],['AGE','SEX'],['AGE','RACE'],['RACE','SEX'],['RACE','OWNERSHP'],['SEX','OWNERSHP']]
  l1errs2tot=0
  l1errs3tot=0
  ...
  #fnames=['AGE','OWNERSHP']
  for fnames in fnameses:
    l1errs2, fairerrs2=red_errs2(hist1,fnames,2)
    l1errs3, fairerrs3=red_errs2(hist1,fnames,3)
    l1errs6, fairerrs6=red_errs2(hist1,fnames,6)
    l1errs2tot+=l1errs2
    l1errs3tot+=l1errs3
    l1errs6tot+=l1errs6
    fairerrs2tot+=fairerrs2
    fairerrs3tot+=fairerrs3
    fairerrs6tot+=fairerrs6
  print('Redact L1 Errors for ',str(puma),'\n k=2:',l1errs2tot/6,'; k=3',l1errs3tot/6,'; k=6',l1errs6tot/6)
  print('Redact Fairness Errors for ',str(puma),'\n k=2:',fairerrs2tot/6,'; k=3',fairerrs3tot/6,'; k=6',fairerrs6tot/6)
...
'''
  ...
  #plt.savefig('DP_Fair_'+str(fnames)+'.pdf',format='pdf',dpi=300)
  sns.boxplot(data=pd.DataFrame({'1':np.array(l1errs1tot)/6, '2':np.array(l1errs2tot)/6, '3':np.array(l1errs3)/6, '5':np.array(l1errs5)/6})).set(title=str(puma)+' DP L1')
  plt.savefig('3DP_L1_'+str(puma)+'.pdf',format='pdf',dpi=300)
  plt.clf()
  sns.boxplot(data=pd.DataFrame({'1':np.array(fairerrs1tot)/6, '2':np.array(fairerrs2tot)/6, '3':np.array(fairerrs3tot)/6, '5':np.array(fairerrs5tot)/6})).set(title=str(puma)+' DP Fair')  
  plt.savefig('3DP_Fair_'+str(puma)+'.pdf',format='pdf',dpi=300)
  plt.clf()
  #fnameses=[['AGE','RACE'],['RACE','SEX'],['RACE','OWNERSHP'],['SEX','OWNERSHP']]
  #fnames=['AGE','OWNERSHP']
  for fnames in fnameses:
  ...
  #plt.savefig('RM_Fair_'+str(fnames)+'.pdf',format='pdf',dpi=300)
  #plt.clf()
  sns.boxplot(data=pd.DataFrame({'2':np.array(l1errsx2tot)/6, '3':np.array(l1errsx3tot)/6, '6':np.array(l1errsx6tot)/6})).set(title=str(puma)+' Rand Merge L1')
  plt.savefig('3RM_L1_'+str(puma)+'.pdf',format='pdf',dpi=300)
  plt.clf()
  sns.boxplot(data=pd.DataFrame({'2':np.array(fairerrsx2tot)/6, '3':np.array(fairerrsx3tot)/6, '6':np.array(fairerrsx6tot)/6})).set(title=str(puma)+' Rand Merge Fair')
  plt.savefig('3RM_Fair_'+str(puma)+'.pdf',format='pdf',dpi=300)
  plt.clf()
'''
#perdbanalysis()
#perpumaanalysis()
